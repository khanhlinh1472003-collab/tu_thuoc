      ...
      if (state != prevState[i]) {
        ...
        if (!state) {
          ...
          if (isEarly) {
            ...
            if (currentReminderState == STATE_ACTIVE && activeReminderBox == i) {
                currentReminderState = STATE_IDLE;
                activeReminderBox = -1;
                drawClock(true); // Cập nhật lại màn hình TFT, thoát cảnh báo
                updateLED();
                String alertMsg = "{\"type\":\"taken\",\"box\":" + String(i + 1) + "}";
                mqtt.publish("esp/alert", alertMsg.c_str(), false);
                Serial.println("[REMINDER] Uong thuoc dung gio!");
            }
            playEmptyBoxVoice(i);
          }
        } else {
