...
function markTaken(boxId, source = 'auto') {
  ...
  const entry = {
    message: `Hộp ${boxId}: Đã uống thuốc (${source === 'manual' ? 'thủ công' : 'tự động'})`,
    type: 'taken',
    boxId: boxId,
    date: todayStr(),
    takenAt: fmtTime(),
  ...
  push(activityLogRef, entry)
    .then(() => {
      addLog(`Đã ghi nhận uống thuốc Hộp ${boxId} thành công.`, 'ok');
    })
  ...
  if (topic === 'esp/alert') {
    try {
      ...
      if (data.type === 'early') {
        ...
        addLog(`ESP32: Đã lấy thuốc hộp ${data.box} đúng giờ!`, 'ok');
      }
    } catch (e) {
