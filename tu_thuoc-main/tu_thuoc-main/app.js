...
const doseLogRef = ref(db, `accounts/${currentAccount}/doseLog`);
  ...
  if (boxIdx !== -1) {
    ...
    if (prevStatus === 'has' && box.status === 'empty') {
      ...
      if (assigned && !isNearScheduledTime(assigned.time)) {
        ...
        // Toast cũ: showWarningToast(box.id, assigned.time);
        showAlertModal('Cảnh báo lấy thuốc sớm!', `Bạn vừa lấy thuốc tại Hộp ${box.id} khi chưa đến giờ uống. Vui lòng trả lại!`, 'early', box.id);
      } else {
  ...
  if (topic === 'esp/alert') {
    try {
      ...
      if (data.type === 'early') {
        ...
        if (state.activeAlertBox === data.box) {
           closeAlertModal();
           addLog(`Thuốc hộp ${data.box} đã được trả lại. Tắt cảnh báo.`, 'ok');
        }
      }
    } catch (e) {
